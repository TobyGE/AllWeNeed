...
- 论文、preprint、研究 Blog、实验结果和 benchmark 只能作为证据，不能直接包装成 Explore。研究主导的条目必须另有现实部署/采用、明确公司或机构动作，或不同发布者的独立复现/验证；否则不入选。标题和 thesis 必须是可证伪的产业、产品、组织或行为变化，不能只是论文讲解或研究结论改写。
