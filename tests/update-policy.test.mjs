...
test("official material events enter the immediate fast lane", () => {
  ...
  const plan = buildUpdatePlan({
    candidates: [release],
    now,
    fallbackLastCycleAt: "2026-08-01T11:55:00.000Z",
  });
...
test("standard events wait no longer than two hours", () => {
  const update = item({
    title: "Cloud infrastructure changelog update",
    firstSeenAt: "2026-08-01T09:59:00.000Z",
  });
  const plan = buildUpdatePlan({
    candidates: [update],
    now,
    scheduleState: {
      laneProcessedAt: { standard: "2026-08-01T11:30:00.000Z" },
    },
  });
  assert.equal(candidateUpdateLane(update), "standard");
  assert.deepEqual(plan.dueLanes, ["standard"]);
  assert.equal(updateIntervalsMinutes.standard, 120);
});

test("Explore is batched for six hours", () => {
  const essay = item();
  ...
  assert.equal(queued.nextDueAt, "2026-08-01T16:00:00.000Z");

  const due = buildUpdatePlan({
    candidates: [
      item({ firstSeenAt: "2026-08-01T05:59:00.000Z" }),
    ],
    now,
    scheduleState: {
      laneProcessedAt: { explore: "2026-08-01T10:00:00.000Z" },
